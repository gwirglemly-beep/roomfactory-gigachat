...
async function generateImage(token, description, stylePrompt, meta) {
  ...
  const fullPrompt = [
    ...
    'Планировка помещения (строго обязательно к соблюдению): сохрани точно такую же форму и пропорции комнаты, высоту потолков, такой же ракурс и точку обзора камеры, и такое же количество и расположение окон и дверей, как описано ниже: ' + description,
    'КРИТИЧЕСКИ ВАЖНО про окна и двери: нарисуй ровно столько окон, сколько указано в описании выше — ни одним больше. Если в описании одно окно, в результате должно быть только одно окно, не два и не три. То же самое для дверей и дверных проёмов — только те, что перечислены в описании. Строго запрещено добавлять любые дополнительные окна, двери, стены, ниши или архитектурные элементы, которых нет в описании.',
