...
async function describeImage(token, fileId) {
  const resp = await fetch(API_BASE + '/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: 'Ты помогаешь дизайнеру интерьера. Очень подробно опиши планировку комнаты на прикреплённом фото, по пунктам: ' +
          '1) Тип и примерная форма помещения (квадратная, прямоугольная, вытянутая), примерное соотношение ширины и длины. ' +
          '2) Сколько окон, на какой именно стене каждое расположено (левая, правая, дальняя, ближняя к камере), примерный размер окон. ' +
          '3) Где находится дверь или дверной проём — на какой стене, ближе к какому углу. ' +
          '4) Высота потолков на глаз (стандартная, высокая, низкая). ' +
          '5) Ракурс и точка съёмки — откуда снята комната (от двери, от окна, из угла). ' +
          '6) Какие крупные архитектурные элементы есть ещё (ниши, колонны, балки, встроенные шкафы). ' +
          'Пиши только фактическое описание расположения объектов, без оценок и без описания мебели или декора.',
        attachments: [fileId]
      }]
    })
  });
...
async function generateImage(token, description, stylePrompt, meta) {
  ...
  const fullPrompt = [
    ...
    'Планировка помещения (строго обязательно к соблюдению): сохрани точно такую же форму и пропорции комнаты, высоту потолков, и такое же количество и расположение окон и дверей, как описано ниже. Не добавляй новые окна, двери или стены и не меняй их положение и размер: ' + description,
    (meta.roomLabel || meta.colorLabel)
      ? 'Ещё раз главное условие: это ' + (meta.roomLabel ? meta.roomLabel.toLowerCase() : 'комната') + (meta.colorLabel ? ' в цветовой гамме "' + meta.colorLabel + '"' : '') + (requiredFurniture ? ', с реальной обстановкой (' + requiredFurniture + ')' : '') + '.'
      : ''
  ].filter(Boolean).join(' ');
